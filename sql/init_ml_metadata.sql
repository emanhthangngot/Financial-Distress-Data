-- Phase 2 ml_metadata schema. Runs against phase2-postgres (pgvector/pgvector:pg16),
-- a separate Postgres instance from Phase 1's project_metadata (AGENTS.md: the two
-- schemas never cross-write). Mirrors init_project_metadata.sql's style.
...
CREATE TABLE IF NOT EXISTS ml_metadata.label_table (
...
CREATE TABLE IF NOT EXISTS ml_metadata.feast_registry_revision (
...
CREATE TABLE IF NOT EXISTS ml_metadata.stream_feature_checkpoint (
...
CREATE TABLE IF NOT EXISTS ml_metadata.rag_document (
...
CREATE TABLE IF NOT EXISTS ml_metadata.rag_chunk (
    chunk_id TEXT PRIMARY KEY,
    document_hash TEXT NOT NULL REFERENCES ml_metadata.rag_document (document_hash),
...
CREATE INDEX IF NOT EXISTS rag_chunk_embedding_hnsw
    ON ml_metadata.rag_chunk USING hnsw (embedding vector_cosine_ops);

-- Governance quarantine (src/llm/data_governance.py, phase-04B).
CREATE TABLE IF NOT EXISTS ml_metadata.rag_quarantine (
    chunk_id TEXT PRIMARY KEY,
    document_hash TEXT,
    content_hash TEXT,
    source_uri TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS ml_metadata.rag_ingestion_run (
...
-- Phase 2 DQ results (AGENTS.md: never cross-written with project_metadata).
CREATE TABLE IF NOT EXISTS ml_metadata.data_quality_result (
