...
EXPLICIT_IMPLEMENTATION: dict[str, tuple[str, str, str]] = {
    ...
    "LLM-validation-verification-validation-verification": (
        "llm_engineer",
        "source",
        "tests/platform/requirements/test_llm_ac_10_validation.py",
    ),
    "LLM-validation-verification-c-s-d-ng-k-thu-t-equivalence-p": (
        "llm_engineer",
        "source",
    ...
    "LLM-ci-cd-agent-k-o-d-li-u": (
        "llm_engineer",
        "source",
        ".github/workflows/phase2-agent-feature.yaml",
    ),
    ...
    "LLM-ci-cd-job-1": (
        "data_engineer",
        "source",
        ".github/workflows/phase2-stream-feature-offline.yaml",
    ),
    "LLM-ci-cd-job-2": (
        "data_engineer",
        "source",
        ".github/workflows/phase2-stream-feature-online.yaml",
    ),
    "LLM-routing-gateway-c-c-service-c-n-c-hide-ng-sau-": (
        "platform_operator",
        "gitops",
...
def _parse_csv(csv_path: Path, track: str) -> list[dict[str, object]]:
    ...
    for source_row_index, raw_row in enumerate(all_rows[1:], start=1):  # skip header
        ...
        test = "pytest tests/platform -k '" + rid + "'"
        ...
        validation_command = (
            f"pytest tests/platform/requirements/test_{validation_slug}.py -k '{rid}'"
        )
        out.append(
            {
                "rubric_id": rid,
