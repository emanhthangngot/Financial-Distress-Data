...
the platform feature, label and drift-reference tables use Apache Iceberg semantics
...
| Nessie | Rejected for now: Git-like data branches are not required by the the platform workflow. |
...
migration. the platform Parquet evidence remains frozen and is not migrated into
the platform Iceberg tables.
