...
- versions: coordinator/feature-agent/drift-agent and MCP images from the the platform Artifact Registry digests; Prometheus recording rules from platform/observability; request path uses qwen2.5-0.5b-instruct
- command: start `kubectl -n agents-sandbox port-forward svc/coordinator 18080:80` and `kubectl -n monitoring port-forward svc/monitoring-kube-prometheus-prometheus 19090:9090`; POST the nested `feature_request`/`drift_request` payload in the companion token-metrics artifact, then query `platform:agent_calls_total:rate5m`, `platform:mcp_tool_calls_total:rate5m`, and `platform:agent_invocation_failures_total:rate5m` through the Prometheus API
...
```text
query={__name__="platform:agent_calls_total:rate5m"}
[
...
query={__name__="platform:mcp_tool_calls_total:rate5m"}
[
...
query={__name__="platform:agent_invocation_failures_total:rate5m"}
[
...
The recording rules `platform:agent_calls_total:rate5m`,
`platform:mcp_tool_calls_total:rate5m`, and
`platform:agent_invocation_failures_total:rate5m` are the live rule families
