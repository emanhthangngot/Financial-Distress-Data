...
- command: `kubectl apply -f platform/security/lakehouse-proof-sealed-secret.yaml` then `kubectl get secret lakehouse-sealed-secret-proof -n default -o jsonpath='{.data.proof-token}' | base64 -d`
...
- actual_result: `sealedsecret.bitnami.com/lakehouse-sealed-secret-proof created`; `kubectl get sealedsecret -n default` shows it reconciled (age 15s); decoded `proof-token` = `lakehouse-nonproduction-proof`
