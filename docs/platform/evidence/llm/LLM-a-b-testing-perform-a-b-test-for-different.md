...
- Dashboard/config inspection -> finds `platform-ab-dashboard` and `fd-agent-model-v1`/`fd-agent-model-v2` -> the A/B split has dashboard and per-version model-config resources available for review.
