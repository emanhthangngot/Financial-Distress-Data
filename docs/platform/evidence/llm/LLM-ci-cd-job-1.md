...
Proves `.github/workflows/platform-stream-feature-offline.yaml` calling the
...
(`dags/platform/phase2_stream_feature_offline.py`).
...
- command: `git push origin dev` (merge of PR #54) triggering `platform-stream-feature-offline.yaml` on the `push` event; workflow run watched via `gh run list --branch dev` and `gh run view <id> --json jobs`
...
to `success`, all in `.github/workflows/platform-ci.yaml` / the three caller
workflows:
