...
- command: (1) `curl http://127.0.0.1:19090/api/v1/targets` via `kubectl -n monitoring port-forward svc/monitoring-kube-prometheus-prometheus`; (2) Grafana login + its own datasource-proxy query for the same PromQL used by the `the platform LLM observability` dashboard's "Web API request rate" panel
...
serviceMonitor/platform-data/feature-mcp/0  http://10.20.0.137:8000/metrics  up
serviceMonitor/platform-data/drift-mcp/0    http://10.20.0.136:8000/metrics  up
```
...
The dashboard `db/phase-2-llm-observability` (uid `platform-llm-observability`) exists in the live Grafana instance and its "Web API request rate" panel uses this exact query — this is the query Grafana itself runs when rendering that panel, executed non-interactively for reproducibility.
