...
Proves `.github/workflows/platform-rag-pipeline.yaml` calling the reusable
`.github/workflows/platform-ci.yaml` template runs the full sign → push →
...
- command: `git push origin dev` (merge of PR #61/#62 into `dev`, then `dev`→`main` merge PR #63) triggering `platform-rag-pipeline.yaml`; watched via `gh run list --branch dev` and `gh run view <id> --json jobs`
...
$ kubectl get cronjob rag-pipeline -n platform-data -o jsonpath='{.spec.jobTemplate.spec.template.spec.containers[0].image}'
