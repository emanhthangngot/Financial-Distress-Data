...
- actual_result: 400 financial_statement rows in -> 400 label rows out; schema matches exactly (verified by `tests/platform/pipelines/test_label_pipeline.py::test_build_labels_returns_exact_schema`); sample below
...
`write_labels_postgres` upserts into `ml.label_table`
...
in this sandbox (see `tests/platform/pipelines/test_label_pipeline.py`'s
`_FakeConn`-based unit tests for the upsert-logic proof, and
`dags/platform/phase2_label_drift_build.py` for the real wiring).

## Test coverage

`.venv/bin/python -m pytest tests/platform/pipelines/test_label_pipeline.py -q`
