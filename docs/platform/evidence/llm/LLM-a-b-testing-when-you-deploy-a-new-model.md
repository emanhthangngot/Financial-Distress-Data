...
- Agent configuration -> exposes `fd-agent-model-v1` and `fd-agent-model-v2` -> agents can be evaluated against separate model configurations while `platform-ab-dashboard` provides the dashboard resource.
