# the platform Low-Level Design — Class Contracts
...
- Executable slice: [`src/llm/embedding_registry.py`](../../src/llm/embedding_registry.py)
  ...
  lock. `tests/platform/verification/test_llm_novel_ideas.py` proves the old and
...
```bash
.venv-platform/bin/python -m pytest -q tests/platform/verification/test_llm_novel_ideas.py
```

## Contract Enforcement

- Signature stubs live in `src/ml/contracts.py` and `src/llm/contracts.py`.
- `tests/platform/test_rubric_matrix.py::TestClassContracts` already imports both
