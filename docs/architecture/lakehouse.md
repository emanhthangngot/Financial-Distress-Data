# the platform Architecture Overview: Financial Distress Data Engineering System

Tài liệu này là bản tả chi tiết (As-Built Specification) về toàn bộ kiến trúc, hạ tầng, luồng xử lý dữ liệu, Data Contracts, kiểm soát chất lượng (DQ), và quản trị Operational Metadata của **the platform (Stage 1 Local-First Lakehouse)** thuộc hệ thống phân tích kiệt quệ tài chính (Financial Distress Analytics System) cho các doanh nghiệp niêm yết tại Việt Nam, đồng thời trình bày **Định hướng Kiến trúc Tổng thể của the platform** (Machine Learning, Agentic RAG & Cloud Evidence Plane).

---

## 1. Nguồn Gốc Quy Chuẩn & Tài Liệu Tham Chiếu (Source of Truth)

Mọi thiết kế và mã nguồn trong the platform tuân thủ thứ tự ưu tiên pháp lý hệ thống sau đây:

1. [AGENTS.md](../../AGENTS.md): Hiến pháp nguyên tắc hoạt động và ranh giới local-first.
2. [mini_coursework.md](../mini_coursework.md): Spec kỹ thuật chi tiết làm căn cứ hiện thực hóa the platform.
...
4. [architecture/data-model.md](../architecture/data-model.md): Spec chi tiết về thiết kế Schema Medallion (Bronze/Silver/Gold), Data Quality rules, Metadata schema và DuckDB SQL contracts.
...
8. [11_rubric_completion_spec.md](../11_rubric_completion_spec.md): Tiêu chí hoàn thành 100% yêu cầu đồ án the platform.
...
the platform được giới hạn nghiêm ngặt trong môi trường **Local-First**, không sử dụng bất kỳ dịch vụ đám mây tính phí nào (như AWS S3, RDS, Glue, Athena, Redshift, EMR, MSK hay Kubernetes):
* Sử dụng **MinIO** thay thế cho AWS S3 với S3A connector (`s3a://financial-distress-lake/`).
* Sử dụng **PostgreSQL** chạy trong Docker làm cơ sở dữ liệu Operational Metadata (schema `ops`).
...
```text
  ...
  | - 100% PyTest Contract Coverage     |                     | - Postgres ops Schema  |
...
| Database: postgres | Schema: ops     |       | Engine: DuckDB httpfs Direct MinIO Reader        |
| File DDL: init_ops.sql              |       | File SQL: duckdb_create_views.sql                |
...
* [vnstock_fixture_adapter.py](../../src/collectors/source_adapters/vnstock_fixture_adapter.py): Adapter chính của the platform. Sinh dữ liệu giả lập có tính toán toán học chính xác (deterministic) cho các công ty niêm yết (VNM, VIC, HPG, FPT, TCB, VCB, MSN, VHM...). Đảm bảo dữ liệu nhất quán qua mỗi lần chạy test mà không cần truy cập internet.
...
* **Soft-Fail**: Lỗi bất thường dữ liệu (VD: `total_assets < 0`, `current_ratio` bất thường). Xử lý: Tách các dòng vi phạm ghi vào bảng `ops.failed_records` trên PostgreSQL, các dòng hợp lệ tiếp tục chảy sang tầng Gold.
...
* [init_ops.sql](../../sql/init_ops.sql): DDL khởi tạo schema `ops` trên PostgreSQL local.
...
CREATE TABLE ops.data_quality_result (
    ...
    run_id VARCHAR(64) REFERENCES ops.pipeline_run_log(run_id),
...
CREATE TABLE ops.failed_records (
...
* [run_lakehouse_real_e2e.py](../../scripts/run_lakehouse_real_e2e.py): Script kích hoạt và theo dõi việc chạy DAG E2E thực tế trên Airflow Docker.
* [check_lakehouse_services.py](../../scripts/check_lakehouse_services.py): Kiểm tra độ sẵn sàng (Healthcheck) của các dịch vụ Docker (Airflow, MinIO, Postgres, Kafka).
* [run_lakehouse_quality_gates.py](../../scripts/run_lakehouse_quality_gates.py): Chạy Quality Gate kiểm tra 100% hợp đồng dữ liệu trước khi xuất bằng chứng.
* [run_lakehouse_dq_failure_probe.py](../../scripts/run_lakehouse_dq_failure_probe.py): Script cố tình bơm dữ liệu lỗi để kiểm chứng tính năng Soft-fail cách ly vào `failed_records`.
* [lakehouse_readiness_report.py](../../scripts/lakehouse_readiness_report.py): Xuất báo cáo tổng quan độ sẵn sàng của the platform cho Reviewer.
* [audit_lakehouse_evidence.py](../../scripts/audit_lakehouse_evidence.py): Đọc và thẩm định toàn bộ file minh chứng trong thư mục [docs/evidence/](../evidence/).

---

## 5. Quy Trình Chống Rò Rỉ Dữ Liệu Theo Thời Gian (Point-In-Time Leakage Guard)

Trong bài toán kiệt quệ tài chính, rò rỉ dữ liệu tương lai (Look-ahead Bias / Point-In-Time Leakage) là lỗi nghiêm trọng làm hỏng mô hình Machine Learning. Hệ thống the platform áp dụng quy tắc quản lý mốc thời gian nghiêm ngặt:
...
Toàn bộ minh chứng thực thi the platform được lưu trữ tại [docs/evidence/](../evidence/) và tài liệu chỉ mục [evidence-index.md](../evidence/README.md).
...
## 7. Kết Luận Sẵn Sàng Cho the platform

Hệ thống the platform đã hoàn thành xuất sắc 100% các tiêu chí Data Engineering Lakehouse. Kiến trúc the platform là một nền tảng **bất biến và đáng tin cậy** (Immutable Data Foundation). 

Nền tảng Gold Parquet trên MinIO sẽ đóng vai trò là nguồn cấp Feature Store đạt chuẩn cho việc huấn luyện mô hình Machine Learning, Agentic RAG và Drift Monitoring ở the platform.

---

## 8. Định Hướng Kiến Trúc Tổng Thể the platform (the platform Architectural Vision & Intent)

Tài liệu thiết kế gốc cho the platform được lưu trữ tại [platform/architecture.md](../platform/architecture.md) và [platform/requirements.md](../platform/requirements.md). the platform xây dựng một hệ thống Machine Learning, Agentic RAG Assistant và Drift Monitoring hoàn chỉnh dựa trên nền tảng Lakehouse the platform.

### 8.1. Mô Hình Kiến Trúc 2 Mặt Phẳng (Two-Plane Architecture Model)

Nhằm tối ưu hóa chi phí vận hành đám mây, the platform áp dụng mô hình thiết kế **Two-Plane Architecture**:
...
### 8.3. Mô Tả Chi Tiết Chức Năng Từng Thành Phần Trong the platform
...
* **Offline Store (MinIO/S3 Parquet)**: Trích xuất lịch sử tính năng từ bảng Gold của the platform để tạo các tập dữ liệu huấn luyện (Training Datasets) có kiểm soát Point-in-Time.
...
### 8.4. Tám Luồng Dữ Liệu Vận Hành Của the platform (8 Numbered Data Flows)
...
### 8.5. Nguyên Tắc Bất Biến Bảo Vệ the platform (the platform Non-Mutation Principle)

* Mã nguồn triển khai the platform được cô lập hoàn toàn trong các thư mục riêng biệt: `src/ml/`, `src/drift/`, `src/llm/`, `src/agents/`, `apps/web/`.
* the platform **tuyệt đối không làm thay đổi hay ảnh hưởng** đến bất kỳ mã nguồn Data Ingestion, PySpark Transforms, Data Quality rules hay Metadata schema nào của the platform.
* the platform tiếp tục vận hành độc lập, cung cấp nguồn dữ liệu sạch bất biến (Immutable Lakehouse Data Foundation) cho the platform tiêu thụ.

---

## 9. Class Contracts & Design Patterns của the platform (Low-Level Design)

Bản thiết kế class contracts được khoá trước khi bắt đầu hiện thực. File contract stubs sống tại [src/ml/contracts.py](../file:///home/pearspringmind/Studying/FSDS/Financial-Distress-Data/src/ml/contracts.py) và [src/llm/contracts.py](../file:///home/pearspringmind/Studying/FSDS/Financial-Distress-Data/src/llm/contracts.py). Spec đầy đủ tại [platform/low-level-design.md](../file:///home/pearspringmind/Studying/FSDS/Financial-Distress-Data/docs/platform/low-level-design.md).
...
## 10. Bốn Ý Tưởng Độc Đáo (Novel Ideas) của the platform

Bốn ý tưởng sáng tạo này được định nghĩa từ trước khi bắt đầu hiện thực theo quy định SDD. Chi tiết đầy đủ tại [platform/novel-ideas.md](../file:///home/pearspringmind/Studying/FSDS/Financial-Distress-Data/docs/platform/novel-ideas.md).
...
* **Đường dẫn bằng chứng**: `docs/platform/evidence/ml/pit-leakage-guard.md`.
...
* **Đường dẫn bằng chứng**: `docs/platform/evidence/ml/reproducibility-manifest.md`.
...
* **Đường dẫn bằng chứng**: `docs/platform/evidence/llm/embedding-hot-swap.md`.
...
* **Đường dẫn bằng chứng**: `docs/platform/evidence/llm/citation-pii-guard.md`.
