...
It is not enterprise-ready; the evidence here should only claim the platform
local-first behavior.

Minimum the platform submission artifacts:
...
- PostgreSQL query export or screenshot for `ops.pipeline_run_log`.
- PostgreSQL query export or screenshot for `ops.data_quality_result`.
- PostgreSQL query export or screenshot for `ops.dataset_freshness`.
- PostgreSQL query export or screenshot for `ops.backfill_request`.
- PostgreSQL query export or screenshot for `ops.source_request_log`.
- PostgreSQL query export or screenshot for `ops.collector_checkpoint`.
...
.venv/bin/python scripts/run_stage1_real_e2e.py \
  ...
  --export-evidence /tmp/lakehouse-e2e-full
```
...
```sql
SELECT * FROM ops.pipeline_run_log ORDER BY created_at DESC LIMIT 20;
SELECT * FROM ops.data_quality_result ORDER BY checked_at DESC LIMIT 20;
SELECT * FROM ops.dataset_freshness ORDER BY checked_at DESC LIMIT 20;
SELECT * FROM ops.backfill_request ORDER BY created_at DESC LIMIT 20;
SELECT source_system, source_endpoint, request_status, count(*)
FROM ops.source_request_log
GROUP BY source_system, source_endpoint, request_status;
SELECT collector_name, source_system, checkpoint_key, checkpoint_value
FROM ops.collector_checkpoint;
SELECT dataset_name, schema_version, schema_json
FROM ops.schema_version_registry
