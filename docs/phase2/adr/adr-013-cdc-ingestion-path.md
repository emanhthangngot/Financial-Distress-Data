...
writes the Phase 2 Iceberg Bronze table through its REST catalog sink. The
initial snapshot and incremental WAL phases are explicit in the source
connector contract.

The path runs in parallel with the existing Phase 1 `generator -> Kafka`
...
- **Reconfigure the Phase 1 Postgres:** rejected because `src/` Phase 1
