...
- Date: 2026-08-07
- Deciders: Phase 2 architecture review, cost owner, platform operator
- Supersedes: ADR-001 (in part), ADR-003; amends ADR-004, ADR-005, ADR-007 (in
  part); defers ADR-006
- Related: `plans/260802-1037-unified-phase2-ml-llm-gitops/plan.md` Session 2
...
- No Phase 2 Python dependency was installed; `.venv` held only `pytest`,
...
rows remain in `docs/phase2/rubric-matrix.csv` unchanged, and
...
- Evidence is produced on Terraform-provisioned GKE, paid from GCP free-trial
  ...
  plainly in `docs/phase2/` and the README. "Designed", "configured",
...
- Three declared GitOps artifact paths are retargeted in both
  `docs/phase2/rubric-matrix.csv` and
