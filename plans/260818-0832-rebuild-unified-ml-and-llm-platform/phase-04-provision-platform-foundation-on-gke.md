...
- [ ] vLLM CPU throughput smoke-tested with explicit thread configuration before phase 6 depends on it
- [ ] **Argo Rollouts** controller installed, providing the progressive-delivery mechanism both tracks' A/B rows depend on
...
Argo Rollouts uses Istio `VirtualService` for canary weights — the most documented
traffic-router path, and the reason sidecar mode simplifies the A/B mechanism
relative to ambient's Gateway API route.
...
10. Install Knative **Serving and Eventing** plus **KServe 0.18+** — both the core controller (`InferenceService`, for the ML track's Triton serving) and the `llmisvc` controller (`LLMInferenceService`/`LLMInferenceServiceConfig`, for llm-d). Replace the old `v0.14.1` pin and its vendored manifests wholesale. Record the new pins in `platform/inference/VERSIONS.md`. Use **`net-istio`** as Knative's networking layer, replacing `net-kourier`, since Istio is now mesh-wide.
   Knative **Eventing** is not optional and not implied by Serving: the ML rubric names it literally — *"Cho real-time drift detection Web API (sử dụng KNative Eventing kết hợp với KServe)"*. Install the Eventing CRDs and an in-memory or Kafka-backed Broker here; phase 5 wires the drift service's Trigger to it.
11. Smoke-test three things on that install before any later phase depends on them: one `InferenceService`, one `LLMInferenceService`, and one Eventing Broker→Trigger delivery. In the same step, benchmark vLLM CPU throughput with explicit `OMP_NUM_THREADS` / thread pinning — the CPU backend has a documented history of exercising a single core regardless of available cores. If throughput is unusable, switch the backend to an OpenAI-compatible llama.cpp server behind the same `LLMInferenceService` and custom-runtime wiring; the CRD, gateway and agent config are unchanged, so this is a backend swap rather than a re-architecture. Decide this here, not in phase 6.
12. Deploy Prometheus, Grafana, Loki, Jaeger, PushGateway and an **OpenTelemetry Collector**; confirm scraping, log shipping and trace collection all work before any application depends on them. The Collector is what makes the trace rows real: Istio ambient emits L4 per-hop spans only, so without application instrumentation a "trace" is a pair of disconnected network hops rather than one request walking API → service → Feast → inference. Services export OTLP to the Collector, which forwards to Jaeger.
13. Rebuild `make gcp-up` / `gcp-down` / `gcp-status` for the new node-pool layout; verify PVCs survive a full hibernate/wake cycle. Add per-group targets (`make up-training`, `up-streaming`, `up-analytic`, `up-governance`) so a window brings up only what it needs — the capacity budget in `plan.md` depends on this being one command, not a manual scaling exercise nobody does under time pressure.
14. Measure and record the real always-on idle vCPU with every windowed group scaled to zero. Compare against the ~16 vCPU floor in the budget. If it exceeds that, cut before phase 5 rather than discovering the ceiling mid-training.
15. Run the Ansible playbook against the VM twice; capture the second run's `changed=0` output.
...
- [ ] `kubectl get svc -A --field-selector spec.type=LoadBalancer` returns exactly one row
