---
phase: 9
title: "platform data plane onto the cluster"
status: cancelled
priority: P2
effort: "2d"
dependencies: [6, 7]
...
> **CANCELLED 2026-08-14 (user decision, ML track dropped).** No LLM rubric row requires platform .ontainerised onto the shared cluster (measured 2026-08-14). Closed only ML rows (~6 pts). `infra/phase1-cluster/` (was untracked, never wired into any active workflow) is now committed as-is in the ML-scaffolding archive commit.
...
# Phase 9: platform data plane onto the cluster
...
plane as platform . **by packaging its existing code, not by changing it**.
...
- Functional: Kafka, MinIO, Airflow, Flink and Postgres run on the cluster under
  Argo CD, with the identical platform .ipeline code producing identical results;
...
platform .vidence. The cluster deployment is an additional runtime target. Keeping
...
evidence VM as the platform .ost for components that are genuinely better off the
...
| Postgres (platform data) | cluster StatefulSet | consumed by cluster workloads |
...
- Modify: `configs/phase2-deployables.yaml` — add the platform .ipeline image
- Create: `tests/platform/test_phase1_cluster_parity.py`
...
1. Build a pipeline image from the existing platform .ource with no source edits.
...
3. Deploy via Argo CD in a dedicated namespace with its own sync wave. Do not
   co-mingle with platform .amespaces — separate namespaces keep the network
   policy and mesh authorization stories clean.
4. Run the platform .ipeline on the cluster and write a parity test comparing its
...
6. Confirm the local `docker compose config` and the platform .uality gate still
   pass unchanged.
...
- [ ] platform .ipeline -> runs on the cluster -> produces outputs matching the recorded local run (parity test green)
- [ ] `docker compose config` and the platform .uality gate -> run locally -> still pass unchanged
- [ ] platform .mages -> admitted by Kyverno -> signed and digest-pinned like everything else
...
- **Parity failure would indicate an environment dependency in platform .ode**,
  which is exactly what the test exists to surface. If it fails, do **not** fix by
  editing platform .ource — fix the container environment.
