...
- Phase 2 RAG CI lint -> checks the configured `src/llm` and RAG/governance test paths with Ruff `0.8.*` -> all checks pass.
- Phase 2 RAG CI formatting -> checks the same paths with Black `24.*` -> all 15 files remain unchanged.
- RAG pipeline tests -> runs `tests/phase2/pipelines -k 'rag or governance' -m 'not slow'` in the Phase 2 environment -> 59 tests pass and 123 are deselected.
...
.venv-phase2/bin/python -m pytest tests/phase2/pipelines -k 'rag or governance' -m 'not slow' -q
