...
description: "Adapt RecSys-MLops' validation-verification evidence-bundle presentation (coverage table, mutation/locust summary files, screenshot checklist, Current Production Sources table) to the LLM track, using already-captured real numbers — no new test/mutation/load runs. Additive only: append to docs/platform/evidence/README.md, add new files under docs/platform/evidence/llm/validation-verification/; existing per-row evidence .md files stay untouched (canonical, must not be rewritten)."
status: completed
priority: P2
...
`docs/platform/evidence/llm/LLM-validation-verification-*.md` with no bundle
...
**Outcome.** `docs/platform/evidence/llm/validation-verification/README.md`
...
Production Sources" table is appended to `docs/platform/evidence/README.md`
...
| Constraint | Evidence |
|---|---|
| Canonical evidence `.md` files "must not be rewritten by a capture run" | `docs/platform/evidence/README.md:3-6` |
| All underlying numbers already exist, real, dated 2026-08-10 | `docs/platform/evidence/llm/LLM-validation-verification-*.md` (read verbatim this session) |
| Raw Locust artifacts already on disk | `docs/platform/evidence/llm/locust-report.html`, `locust_stats.csv`, etc. |
...
1. Reader -> opens `docs/platform/evidence/llm/validation-verification/README.md` -> sees Coverage table, Required Proof list (linking to real test files/lines), Screenshot/artifact checklist, Mutation Summary, Locust Summary — same section order as RecSys, our real numbers.
...
3. Reader -> opens `docs/platform/evidence/README.md` -> finds a new "Current Production Sources" table below the existing content, format/prose above unchanged.
...
- [x] `docs/platform/evidence/llm/validation-verification/README.md` created, RecSys section shape, real numbers, links to the 5 canonical evidence files (every relative link verified with `[ -e ]` this session)
- [x] `docs/platform/evidence/llm/validation-verification/mutation-summary.md` and `locust-sla-summary.md` created, RecSys field format
- [x] `docs/platform/evidence/README.md` gets a new "Current Production Sources" table appended; `git diff` confirms existing prose byte-identical above it
