...
def build_stage1_payload() -> dict[str, int]:
    payload = build_evidence_payload(_bucket())
    return payload.row_counts


def write_stage1_minio_outputs() -> dict[str, int]:
    payload = build_evidence_payload(_bucket())
    write_minio_outputs(payload, _bucket())
    return payload.row_counts


def write_stage1_postgres_metadata() -> str:
    payload = build_evidence_payload(_bucket())
    return write_postgres_metadata(payload)


def run_stage1_duckdb_validation() -> str:
    payload = build_evidence_payload(_bucket())
...
def materialize_stage1_local_evidence() -> dict[str, int]:
    payload = build_evidence_payload(_bucket())
    write_minio_outputs(payload, _bucket())
...
if DAG is not None:
    with DAG(
        dag_id="stage1_local_evidence_pipeline",
