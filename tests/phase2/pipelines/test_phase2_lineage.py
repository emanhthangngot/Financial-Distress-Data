"""Pins src/governance/phase2_lineage.py: configs/phase2-governance.yaml
loads and validates, audit works with no live DataHub server, and
emit_phase2_lineage rejects an unknown pipeline name before ever attempting
...
def test_config_loads_and_validates() -> None:
    model = load_phase2_governance_model()
    model.validate()
...
def test_audit_does_not_import_datahub(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    it raise, so the assertion only passes if audit_phase2_lineage's call
    ...
    def _blocking_import(name, *args, **kwargs):
        if name == "datahub" or name.startswith("datahub."):
            raise AssertionError("audit_phase2_lineage must never import datahub")
        return real_import(name, *args, **kwargs)

    monkeypatch.setattr(builtins, "__import__", _blocking_import)
    report = audit_phase2_lineage()
...
def test_emit_rejects_unknown_pipeline_before_importing_datahub() -> None:
    with pytest.raises(KeyError, match="unknown phase2 pipeline"):
        emit_phase2_lineage("run-1", "not_a_real_pipeline", server="http://localhost:8080")
    assert "datahub" not in sys.modules


def test_audit_narrowed_to_one_pipeline_reports_only_that_pipeline() -> None:
    report = audit_phase2_lineage(pipeline_name="phase2_rag_ingest")
