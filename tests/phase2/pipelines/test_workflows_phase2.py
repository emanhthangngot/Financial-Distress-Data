"""Pins .github/workflows/phase2-*.yaml: the four files exist, parse as
YAML, phase2-ci.yaml is workflow_call, the three callers reference it via
distinct `paths:` triggers, and ci.yml (Phase 1 gate) is byte-unchanged."""
...
def test_all_four_workflow_files_exist() -> None:
...
REQUIRED_INPUTS = ("pipeline_name", "image_context", "dockerfile", "test_selector", "lint_paths")
...
def test_each_caller_has_a_unique_pipeline_name() -> None:
    ...
    for caller in CALLERS:
        parsed = yaml.safe_load(caller.read_text(encoding="utf-8"))
        names.append(parsed["jobs"]["ci"]["with"]["pipeline_name"])
