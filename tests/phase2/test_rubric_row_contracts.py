...
so the matrix's ``test`` field — ``pytest tests/phase2 -k '<rubric_id>'`` —
...
def test_every_validation_command_collects_at_least_one_test() -> None:
    """Every row's `pytest tests/phase2 -k '<rubric_id>'` must select a test.
    ...
    """
    proc = _run("tests/phase2", "--collect-only", "-q")
    assert (
        proc.returncode == 0
...
def test_first_row_validation_command_runs() -> None:
    ...
    rid = RUBRIC_IDS[0]
    proc = _run("tests/phase2", "-k", rid, "-q")
    assert proc.returncode == 0, (
        f"`pytest tests/phase2 -k '{rid}'` failed (rc {proc.returncode}): "
